document.addEventListener('DOMContentLoaded', function() {
  ...
  const availableCommands = [
    ...
    {
      command: 'sl',
      description: 'Run the "sl" command for fun',
    },
  ];
...
break;
  ...
  case 'top':
    displayCommandOutput('CPU usage: 10%, Memory usage: 50%');
    break;

  case 'exit':
    // Handle exit logic here (e.g., close the terminal window)
    displayCommandOutput('Exiting the terminal...');
    break;

  case 'sl':
    displayCommandOutput('Sorry, the "sl" command is not available. Try something else!');
    break;
